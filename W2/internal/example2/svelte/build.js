...
function findPages(dir = '.', sink = []) {
  if (ignorePath.has(dir.replace('./', '').replace('.\\', ''))) return;

  const files = readdirSync(dir).filter(file => file[0] !== '_');
  const svelteFiles = files.filter(file => file.endsWith('.svelte') && statSync(join(dir, file)).isFile());
  svelteFiles.forEach(file => sink.push(join(dir, file)));

  files
    .filter(file => !svelteFiles.includes(file))
    .map(file => join(dir, file))
    .filter(file => statSync(file).isDirectory())
    .forEach(file => findPages(file, sink));

  return sink;
}

const zIdPrefix = `z_placeholder_${Math.floor(Math.random() * 1000000000).toString(16)}_`;
const zReplacer = source => `"${zIdPrefix}${Buffer.from(source).toString('base64')}"`;
