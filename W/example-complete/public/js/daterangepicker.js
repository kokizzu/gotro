...
(function( root, factory ) {
	...
	if( typeof define==='function' && define.amd ) {
		define( [ 'moment', '../lib/jquery', 'exports' ], function( momentjs, $, exports ) {
	...
	DateRangePicker.prototype = {
		...
		updateCalendars: function() {
			...
			for( var range in this.ranges ) {
				var tr = this.ranges[ range ];
				var loop_ok = false;
				if( this.timePicker ) {
					if( this.startDate.isSame( tr[ 0 ] ) && this.endDate.isSame( tr[ 1 ] ) ) loop_ok = true;
				} else {
					//ignore times when comparing dates if time picker is not enabled
					if( this.startDate.format( 'YYYY-MM-DD' )==tr[ 0 ].format( 'YYYY-MM-DD' ) &&
						this.endDate.format( 'YYYY-MM-DD' )==tr[ 1 ].format( 'YYYY-MM-DD' ) ) loop_ok = true;
				}
				if( loop_ok ) {
					customRange = false;
