function FormMason( id, rec_type, fields, events, vue_options ) {
	...
	for( z = 0; z<fields.length; ++z ) {
		...
		var typ = field.type;
		var key = field.key;
		var props = ZZ( 'name', key ) + ZZ( 'v-model', 'row.' + key ) + ZZ( 'placeholder', field.placeholder ) + ZZ( 'title', field.placeholder );
		var def_val = '';
		switch( typ ) {
			case 'textarea':
			...
			case 'select':
				var sel_opts = DS[ field.sub_type ] || {};
	...
	var default_opts = {
		...
		methods: {
			...
			edit_event: function( row_id ) {
				...
				if( !row_id ) {
					this.row = Object.assign( {}, this.row, default_row );
